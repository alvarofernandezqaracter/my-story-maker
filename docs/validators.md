...
| --- | --- | --- | --- |
| `esquema` | Toda salida de un rol que la versión conserva trae los campos obligatorios que el esquema de su tarea declara para su tipo: los que aparecen en todos sus ejemplos de ese tipo. El capítulo que el backend puso en el envoltorio cuenta como presente | El cuerpo guardado, con el capítulo de su envoltorio al lado, y el `esquema.json` de la tarea | Que el valor de cada campo sea bueno: solo que está |
...
| La puerta del esquema da por presente el capítulo del envoltorio, y un campo que falta en algún ejemplo de su tipo es opcional (SPEC1 RF-207, RF-208) | Dos ejemplos de un tipo con un campo en uno solo; y una obra fingida entera cuyos eventos no repiten el capítulo y cuyo paso del tiempo no trae objeto, que pasa, frente a otra con un evento sin sujeto, que falla | `prueba` |
...
| La escena de lo que devuelve un encargo la pone el backend: la del encargo, o una del capítulo (SPEC1 RF-216, D-98) | Una crítica de un encargo de capítulo a `esc_0007`, que queda sin escena, frente a otra a una escena del capítulo, que se respeta; la de un encargo de escena, que lleva la del encargo; y un `Beat` del Planificador, que conserva la suya | `prueba` |
| El material `canon` no trae las `Fuente`, que crecen con la obra; `fuentes_recogidas` sí (SPEC1 RF-217, D-99) | Con una fuente y una práctica en el almacén, el canon trae solo la práctica y `fuentes_recogidas`, la fuente | `prueba` |
| El Contable recibe el `id` y el nombre de cada personaje y lugar del canon, no inventa `id` y escribe un `viaja_a` en cada cambio de lugar (SPEC1 RF-218, D-100) | La ventana de `plegar` con el reparto y los lugares, sin los demás atributos de las fichas; y el prompt que lo dice | `prueba` |
