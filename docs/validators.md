...
| Las tareas de una tanda corren a la vez sin pasar de su anchura, con el mismo recorrido que en serie, y la tanda cierra entera aunque una detenga la obra (SPEC1 RF-13, D-51) | Ejecutor fingido que declara `simultaneo` y cuenta las tareas abiertas por paso; el mismo capítulo en serie y en paralelo; un paso de `detener_obra` que falla en todas sus escenas | `prueba` |
