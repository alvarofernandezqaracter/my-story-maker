...
| Contable de estado | Estado en N-1 ya materializado, texto aceptado del capítulo N, vocabulario de tipos de evento, el marco temporal del capítulo —la época, la última fecha que cerró y el lugar y el instante del plan para cada escena— y el `id` y el nombre de cada personaje y lugar del canon | `EventoEstado` del capítulo N, con fecha —ISO parcial— y lugar resultantes ya calculados y los personajes presentes, y estado en N |
...
| Contable de estado | Estado en N-1, texto aceptado del capítulo N, vocabulario de eventos, marco temporal del capítulo, reparto y lugares con su `id` | El plan salvo el marco de sus escenas, críticas, el resto de cada ficha del canon | Transcribe hechos ocurridos, no los interpreta; el año lo toma del marco cuando el texto no lo da; copia los `id` del reparto y escribe un `viaja_a` en cada cambio de lugar |
