...
Lean, el juez externo que puntúa una versión terminada con una rúbrica que
vive fuera del repositorio, los briefs con que se prueba el sistema entero, y
...
| RF-201 | **Los briefs de prueba** viven en `backend/briefs-de-prueba/`, uno por fichero JSON con `para_que`, `se_espera` y `brief`. Cada entrada de `se_espera` nombra una comprobación —`puerta.<validador_de_la_puerta>`, `gancho.<gancho>` o `juez.<criterio>`—, su resultado del vocabulario cerrado `resultado_esperado` —`pasa`, `falla`, `puede_fallar`, `se_puntua`, `no_se_puntua`— y por qué. Una prueba que no gasta los carga todos, valida cada `brief` contra el modelo de `POST /obras` y cada comprobación contra los vocabularios del código. Hay al menos uno adversario de inyección y uno diseñado para provocar una incoherencia temporal que solo ve Lean | `prueba` |
...
15. **El juez de la novela y los briefs.** Sin gastar: sin rúbrica, el juicio
...
del cambio del lector y el PDF, lo que enumera §4.18; y del juez de la novela y
los briefs de prueba, lo que enumera §4.21.
