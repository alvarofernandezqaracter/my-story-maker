...
def test_la_anchura_de_tanda_se_calcula_rol_por_rol() -> None:
    ...
    3 191 tokens con el subagente aislado del repositorio, y eso ocupa techo
    ...
    assert tabla["verificador_de_continuidad"] == 6
    assert tabla["juez_de_rubrica"] == 8
    assert tabla["arquitecto_de_arcos"] == 2
    assert tabla["planificador"] == 3
    assert tabla["redactor"] == 5
...
def test_veinte_comprobaciones_de_verificador_son_cuatro_tandas_de_seis_como_mucho() -> None:
    encargos = guion.expandir(
    ...
    assert [len(tanda) for tanda in tandas] == [6, 6, 6, 2]
...
def test_la_tanda_la_marca_el_rol_mas_caro() -> None:
    ...
    assert presupuesto.anchura_de_tanda([caro, barato]) == 6
...
def test_el_techo_cuenta_solo_la_entrada_pero_toda_la_entrada() -> None:
    assert presupuesto.pico_admisible(80_000, 16_500) is True
    assert presupuesto.pico_admisible(80_000, 16_501) is False
