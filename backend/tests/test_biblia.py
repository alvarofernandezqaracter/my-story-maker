...
def test_las_rutas_nuevas_son_de_lectura(cliente: TestClient) -> None:
    """Ninguna de las dos escribe. La lista entera de escrituras la vigila
    `test_api.py`; aqui solo importa que estas dos no esten en ella."""
    escrituras = operaciones_de_escritura(cliente.app)  # type: ignore[arg-type]

    assert not [ruta for ruta in escrituras if "hechos" in ruta or "cronologia" in ruta]
