...
distintos no comparten ningun presente. -/
abbrev UnSoloLugar (a b : Suceso) : Prop :=
...
def NadieEnDosSitios (c : List Suceso) : Prop := ∀ a ∈ c, ∀ b ∈ c, UnSoloLugar a b
...
instance (c : List Suceso) : Decidable (NadieEnDosSitios c) :=
  inferInstanceAs (Decidable (∀ a ∈ c, ∀ b ∈ c, UnSoloLugar a b))
...
theorem sin_datos_no_contradice (b : Suceso) :
    Precede ⟨0, 0, 99999999, 0, [], 0⟩ b ∧ EdadCoherente ⟨0, 0, 99999999, 0, [], 0⟩ ∧
      UnSoloLugar ⟨0, 0, 99999999, 0, [], 0⟩ b ∧ NoReaparece ⟨0, 0, 99999999, 0, [], 0⟩ b := by
  refine ⟨?_, ?_, ?_, ?_⟩
