...
| `docs/architecture.md` | Cómo está construido el sistema: las tres capas y su regla de acoplamiento, el censo de once agentes con sus tareas y permisos, las entidades de producción (`Plan`, `Borrador`, `Crítica`, `Revisión`, `Decisión`, `EventoEstado`, `Resumen de capítulo`, `Traza`), las tres memorias y el presupuesto de contexto, la orquestación por guion y el ciclo de vida del capítulo, el bucle de control de calidad, la tabla de gobierno por entidad y las decisiones abiertas. Es el documento del *cómo*. |
